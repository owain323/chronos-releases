...
import { createRevenue, updateRevenue, deleteRevenue } from "../services/RevenueService";
...
export const costRouter = router({
    ...
    create: permissionProcedure("finance.edit")
      .input(z.object({
        projectId: z.number(), name: z.string().min(1),
        amount: z.string().regex(/^\d+(\.\d{1,2})?$/),
        category: z.string().min(1), notes: z.string().optional(),
    ...
    update: permissionProcedure("finance.edit")
      .input(z.object({
        ...
        amount: z.string().regex(/^\d+(\.\d{1,2})?$/),
        category: z.string().min(1), notes: z.string().optional().nullable(),
        date: z.string().optional(),
      }))
      .mutation(async ({ input, ctx }) => {
        await requireEntityAccess("cost", input.id, ctx.user.id);
        const result = await db.updateCostEntry(input.id, input);
        recordAudit({ userId: ctx.user.id, action: "update", entity: "costs", entityId: input.id });
        return result;
      }),
