...
async function downloadWecomMedia(
  ...
  try {
    ...
    if (contentType === "application/json" || buffer[0] === 0x7b) {
      try {
        ...
        if (json.errcode && json.errcode !== 0) {
          logger.warn(
            { ctx: "bot" },
            `[media] WeChat API error: errcode=${json.errcode} msg=${json.errmsg}`
          );
          return null;
        }
      } catch {
    ...
    logger.warn(
      { ctx: "bot" },
      `[media] download exception: ${e instanceof Error ? e.message : String(e)}`
    );
    return null;
  }
}
...
async function getWecomAccessToken(): Promise<string | null> {
  if (_wecomToken && _wecomToken.expiresAt > Date.now() + 60000)
...
export async function handleBotMedia(
  ...
  const ext =
    path.extname(basename) !== ".bin" && path.extname(basename).length > 1
  ...
  if (!assertMagicByte(tempPath, originalName)) {
    ...
    logger.warn(
      { ctx: "bot" },
      `[media-handler] magic-byte rejected: ${originalName} (${dl.contentType})`
    );
...
export function buildInboxReply(botUserId: string): string {
  ...
  for (const item of items) {
    const label = item.mime?.startsWith("image/")
      ? "图片"
